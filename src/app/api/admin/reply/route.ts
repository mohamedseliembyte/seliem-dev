...
export async function POST(req: NextRequest) {
  ...
  // Mark human takeover so the AI stays paused until explicitly handed back.
  await supabase.from('conversations').update({ status: 'human', human_takeover: true, updated_at: new Date().toISOString() }).eq('id', conversationId)

  // Notify the client by email if they're offline (shared logic with AI replies).
  // Force-send for human replies since the rep is intentionally reaching out.
  void maybeEmailClientReply(conversationId, content)
